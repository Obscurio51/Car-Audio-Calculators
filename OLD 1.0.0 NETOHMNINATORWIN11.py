...
# Mantiene tutte le funzioni, layout, bottoni, logica, audio opzionale, export CSV, annulla, ecc.
...
###    #### ##########     ###              ########  ###    ### ###       ### ###    #### ########### ###    #### ###     ### ###     ########  ###    ###      
        ...
        btn_export = QtWidgets.QPushButton("Esporta CSV")
    ...
    def export_csv(self):
        ...
        for i in range(root.childCount()):
            it = root.child(i)
            vals = [it.text(c) for c in range(3)]
            items.append(vals)
        if not items:
            QtWidgets.QMessageBox.information(self, "Export", "Nessun dato da esportare")
            return
        fn, _ = QtWidgets.QFileDialog.getSaveFileName(self, "Save CSV", "", "CSV files (*.csv)")
        if not fn:
            return
        try:
            ...
            QtWidgets.QMessageBox.information(self, "Export", f"Esportato: {fn}")
        except Exception as e:
