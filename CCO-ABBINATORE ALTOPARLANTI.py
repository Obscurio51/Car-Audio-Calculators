...
'''
    ...
    def plot_tree_matplotlib(self, node, title=None):
        ...
        # --- 1. Calcolo Elettrico sull'albero binario originale ---
        def get_z(n):
            if n[0] == 'leaf': return float(n[1])
            a, b = get_z(n[1]), get_z(n[2])
            if n[0] == 'S': return a + b
            return 1.0 / (1.0 / (a or 1e-9) + 1.0 / (b or 1e-9))
        ...
        def compute_p(n, v):
            if n[0] == 'leaf':
                leaf_p[id(n)] = v*v/float(n[1])
                return
            if n[0] == 'S':
                z1, z2 = get_z(n[1]), get_z(n[2])
                # Ripartizione tensione in serie: V1 = Vtot * (Z1 / Ztot)
                i = v / ((z1 + z2) or 1e-9)
                compute_p(n[1], i * z1)
                compute_p(n[2], i * z2)
            else: # Parallelo
        ...
        # --- 2. Appiattimento con mantenimento degli ID originali ---
        # Questo garantisce che ogni scatola disegnata sappia esattamente a quale 
        # nodo dell'albero binario corrisponde, evitando scambi di watt.
        def flatten_tree(n):
            if n[0] == 'leaf': 
                return ['leaf', n[1], id(n)] # Manteniamo l'ID univoco
            op = n[0]
            children = []
            def collect(sub):
                if sub[0] == op:
                    ...
                    children.append(flatten_tree(sub))
            collect(n)
            if op == 'S':
                def sort_key(c):
                    if c[0] == 'leaf': return 0
                    if c[0] == 'P': return 2
                    return 1
                children.sort(key=sort_key)
            return [op, children, id(n)]

        flat_node = flatten_tree(node)

        # --- 3. Layout (Parametri identici all'originale) ---
        LEAF_W, LEAF_H = 3.2, 1.6
        NODE_R = 0.45
        GAP_X, GAP_Y = 1.0, 1.2
        WIRE = 0.7
        node_dims = {}
        ...
        def assign(n, cx, y_top):
            ...
            if n[0] == 'leaf':
                pos[nid] = (cx, y_top)
                return
            node_cy = y_top - NODE_R
            pos[nid] = (cx, node_cy)
            child_y_top = y_top - 2 * NODE_R - WIRE
            if n[0] == 'P':
        ...
        assign(flat_node, 0.0, -0.3)

        # --- 4. Disegno Matplotlib (Stesso stile precedente) ---
        ...
        def top_center(n):
            nid = id(n)
            if n[0] == 'leaf': return pos[nid]
            return pos[nid][0], pos[nid][1] + NODE_R

        def bot_center(n):
            nid = id(n)
            if n[0] == 'leaf': return pos[nid][0], pos[nid][1] - LEAF_H
            return pos[nid][0], pos[nid][1] - NODE_R
        ...
        def draw_net(n):
            ...
            if n[0] == 'P':
                ...
                ax.plot([cx, cx], [cy - NODE_R, y_bus], color='white', lw=2, zorder=2)
                ax.plot([min(t[0] for t in tops), max(t[0] for t in tops)], [y_bus, y_bus], color='white', lw=2, zorder=2)
                for ctx, cty in tops:
                    ax.plot([ctx, ctx], [y_bus, cty], color='white', lw=2, zorder=2)
            else: # S
                for i, c in enumerate(children):
                    ctx, cty = top_center(c)
                    if i == 0:
                        ax.plot([cx, cx], [cy - NODE_R, cty], color=NEON_GREEN, lw=3, zorder=2)
                    else:
                        pbx, pby = bot_center(children[i - 1])
                        ax.plot([pbx, pbx], [pby, cty], color=NEON_GREEN, lw=3, zorder=2)
            for c in children: draw_net(c)

        # Elementi fissi (Sorgente e Cavi)
        amp_y = 1.2
        ax.text(0, amp_y, f"SORGENTE\n{input_watts:g}W", color='black', ha='center', va='center', 
                fontweight='bold', fontsize=9, bbox=dict(facecolor='#FFD800', edgecolor='white', boxstyle='round,pad=0.4'), zorder=20)
        _, filo_top = top_center(flat_node)
        ax.plot([0, 0], [amp_y - 0.45, filo_top], color='white', lw=2, zorder=2)

        draw_net(flat_node)

        # Render altoparlanti e nodi (P/S)
        import matplotlib.patches as mpatches
        def render_boxes(n):
            nid = id(n)
            if n[0] == 'leaf':
                cx, y_top = pos[nid]
                original_binary_id = n[2] # Recuperiamo l'ID salvato durante il flatten
                p_val = leaf_p.get(original_binary_id, 0.0)
                rect = mpatches.FancyBboxPatch((cx - LEAF_W/2, y_top - LEAF_H), LEAF_W, LEAF_H,
                                              boxstyle='square,pad=0', facecolor=NEON_GREEN, edgecolor='white', zorder=25)
                ax.add_patch(rect)
                ax.text(cx, y_top - LEAF_H/2, f"{float(n[1]):g} Ω\n{p_val:.1f}W", color='black', 
                        fontsize=9, ha='center', va='center', fontweight='bold', zorder=26)
            else:
                cx, cy = pos[nid]
                color = '#B200FF' if n[0] == 'P' else '#FF0000'
                ax.add_patch(plt.Circle((cx, cy), NODE_R, color=color, ec='white', lw=1.5, zorder=40))
                ax.text(cx, cy, n[0], color='white', fontsize=10, ha='center', va='center', fontweight='bold', zorder=41)
                for c in n[1]: render_boxes(c)

        render_boxes(flat_node)
        ...
        ax.set_ylim(min(all_y) - LEAF_H - margin, amp_y + margin)
