...
'''
        ...
        border_layout.addWidget(self.canvas_widget_frame, 2)
    ...
    def plot_tree_matplotlib(self, node, title=None):
        ...
        try: input_watts = float(self.power_entry.text())
        except: input_watts = 100.0

        # 1. Appiattimento dell'albero binario in albero N-ario
        def flatten_tree(n):
            if n[0] == 'leaf': return n
            op = n[0]; children = []
            def collect(sub):
                if sub[0] == op:
                    collect(sub[1]); collect(sub[2])
                else: children.append(flatten_tree(sub))
            collect(n)
            # Nei nodi SERIE: foglie singole in cima, blocchi parallelo/complessi in fondo
            if op == 'S':
                def sort_key(c):
                    if c[0] == 'leaf': return 0   # foglia singola -> primo (in alto)
                    if c[0] == 'P':    return 2   # blocco parallelo -> ultimo (in basso)
                    return 1                       # serie annidata -> in mezzo
                children.sort(key=sort_key)
            return [op, children]

        flat_node = flatten_tree(node)

        # 2. Calcolo impedenze e potenze sull'albero binario originale
        def get_z(n):
            if n[0] == 'leaf': return float(n[1])
            a, b = get_z(n[1]), get_z(n[2])
            return a + b if n[0] == 'S' else 1.0 / (1.0 / (a or 1e-9) + 1.0 / (b or 1e-9))

        z_tot = get_z(node)
        v_in = (input_watts * max(1e-9, z_tot)) ** 0.5
        leaf_p = {}
        def compute_p(n, v):
            if n[0] == 'leaf': leaf_p[id(n)] = v*v/float(n[1]); return
            if n[0] == 'S':
                z1, z2 = get_z(n[1]), get_z(n[2]); i = v/((z1+z2) or 1e-9)
                compute_p(n[1], i*z1); compute_p(n[2], i*z2)
            else: compute_p(n[1], v); compute_p(n[2], v)
        compute_p(node, v_in)

        # Dimensioni fisse box
        LEAF_W, LEAF_H = 3.2, 1.6   # speaker box
        NODE_R = 0.45                 # raggio cerchio P/S
        GAP_X  = 1.0                  # spazio orizzontale fra speaker in parallelo
        GAP_Y  = 1.2                  # spazio verticale fra speaker in serie
        WIRE   = 0.7                  # lunghezza filo sopra/sotto ogni box

        # 3. Misurazione ricorsiva (ritorna larghezza, altezza del sotto-albero)
        node_dims = {}
        def measure(n):
            nid = id(n)
            if n[0] == 'leaf':
                node_dims[nid] = (LEAF_W, LEAF_H)
                return LEAF_W, LEAF_H
            children = n[1]
            dims = [measure(c) for c in children]
            if n[0] == 'P':
                w = sum(d[0] for d in dims) + GAP_X * (len(children) - 1)
                h = max(d[1] for d in dims) + 2 * WIRE + 2 * NODE_R
            else:  # S
                w = max(d[0] for d in dims)
                h = sum(d[1] for d in dims) + GAP_Y * (len(children) - 1) + 2 * WIRE + 2 * NODE_R
            node_dims[nid] = (w, h)
            return w, h

        measure(flat_node)

        # 4. Assegnazione posizioni
        # pos[id] = (cx, cy_top) dove cy_top è la coordinata y del bordo SUPERIORE del box/nodo
        pos = {}
        node_types = {}

        def assign(n, cx, y_top):
            nid = id(n)
            node_types[nid] = n[0]
            if n[0] == 'leaf':
                pos[nid] = (cx, y_top)
                return
            children = n[1]
            # Il cerchio P/S è posizionato WIRE sotto y_top
            node_cy = y_top - NODE_R   # centro cerchio
            pos[nid] = (cx, node_cy)
            child_y_top = y_top - 2 * NODE_R - WIRE  # inizio area figli

            if n[0] == 'P':
                total_w = node_dims[nid][0]
                curr_x = cx - total_w / 2
                # I figli P partono un WIRE extra più in basso del cerchio
                # così il bus (a metà strada) è sempre visibile
                p_child_y_top = y_top - 2 * NODE_R - WIRE * 2
                for c in children:
                    cw, _ = node_dims[id(c)]
                    assign(c, curr_x + cw / 2, p_child_y_top)
                    curr_x += cw + GAP_X
            else:  # S
                curr_y = child_y_top
                for c in children:
        ...
        assign(flat_node, 0.0, -0.3)   # lascia spazio per il filo dall'amplificatore
        ...
        def top_center(n):
            """Punto di ingresso superiore di un nodo (dove arriva il filo dall'alto)."""
            nid = id(n)
            if n[0] == 'leaf':
                cx, y_top = pos[nid]
                return cx, y_top          # bordo superiore box
            else:
                cx, cy = pos[nid]         # centro cerchio
                return cx, cy + NODE_R    # punto superiore cerchio

        def bot_center(n):
            """Punto di uscita inferiore di un nodo (dove parte il filo verso il basso)."""
            nid = id(n)
            if n[0] == 'leaf':
                cx, y_top = pos[nid]
                return cx, y_top - LEAF_H
            else:
                cx, cy = pos[nid]
                return cx, cy - NODE_R
        ...
        def draw_net(n):
            ...
            if n[0] == 'P':
                # Bus fisso a WIRE sotto il bordo inferiore del cerchio
                # I figli sono WIRE*2 sotto, quindi il filo bus->figlio è lungo WIRE (sempre visibile)
                y_bus = cy - NODE_R - WIRE
                tops = [top_center(c) for c in children]
                x_coords = [t[0] for t in tops]
                # Filo dal cerchio al bus
                ax.plot([cx, cx], [cy - NODE_R, y_bus],
                        color='white', lw=2, solid_capstyle='round', zorder=2)
                # Bus orizzontale
                ax.plot([min(x_coords), max(x_coords)], [y_bus, y_bus],
                        color='white', lw=2, solid_capstyle='round', zorder=2)
                # Fili verticali bus → top di ogni figlio
                for c, (ctx, cty) in zip(children, tops):
                    ax.plot([ctx, ctx], [y_bus, cty],
                            color='white', lw=2, solid_capstyle='round', zorder=2)
                    draw_net(c)

            else:  # S
                # Collega i figli in verticale con linea verde
                for i, c in enumerate(children):
                    ctx, cty = top_center(c)
                    if i == 0:
                        # Filo dal cerchio al primo figlio
                        ax.plot([cx, cx], [cy - NODE_R, cty],
                                color=NEON_GREEN, lw=3, solid_capstyle='round', zorder=2)
                    else:
                        # Filo dal fondo del figlio precedente a questo figlio
                        pbx, pby = bot_center(children[i - 1])
                        ax.plot([pbx, pbx], [pby, cty],
                                color=NEON_GREEN, lw=3, solid_capstyle='round', zorder=2)
                    draw_net(c)

        # Amplificatore in cima
        amp_y = 1.2
        ax.text(0, amp_y, f"SORGENTE\n{input_watts:g}W",
                color='black', ha='center', va='center', fontweight='bold', fontsize=9,
                bbox=dict(facecolor='#FFD800', edgecolor='white', boxstyle='round,pad=0.4'),
                zorder=20)
        # Filo amp → primo nodo
        _, filo_top = top_center(flat_node)
        ax.plot([0, 0], [amp_y - 0.45, filo_top],
                color='white', lw=2, solid_capstyle='round', zorder=2)

        draw_net(flat_node)

        # 6. Disegna speaker (foglie) — DOPO le linee, zorder alto
        def find_flat_node(f_tree, search_id):
            if id(f_tree) == search_id: return f_tree
            if f_tree[0] == 'leaf': return None
            for c in f_tree[1]:
                res = find_flat_node(c, search_id)
                if res: return res
            return None

        # Raccoglie tutti gli id foglia con posizione
        def collect_leaves(n):
            if n[0] == 'leaf': return [(id(n), n)]
            result = []
            for c in n[1]: result += collect_leaves(c)
            return result

        # Mappa ohm → potenza dall'albero binario originale
        # Usiamo una lista per gestire speaker duplicati
        def collect_bin_leaves(orig, lst):
            if orig[0] == 'leaf': lst.append((id(orig), float(orig[1]))); return
            collect_bin_leaves(orig[1], lst); collect_bin_leaves(orig[2], lst)
        bin_leaves = []
        collect_bin_leaves(node, bin_leaves)
        ohm_to_pw = {lid: leaf_p.get(lid, 0.0) for lid, _ in bin_leaves}

        import matplotlib.patches as mpatches
        for nid, fn in collect_leaves(flat_node):
            cx, y_top = pos[nid]
            ohm = float(fn[1])
            # Trova il primo bin_leaf con questo ohm
            p_val = 0.0
            for lid, lohm in bin_leaves:
                if lohm == ohm:
                    p_val = ohm_to_pw.get(lid, 0.0)
                    bin_leaves.remove((lid, lohm))
                    break
            # Rettangolo esplicito: bordo superiore ESATTAMENTE a y_top
            rect = mpatches.FancyBboxPatch(
                (cx - LEAF_W / 2, y_top - LEAF_H), LEAF_W, LEAF_H,
                boxstyle='square,pad=0', linewidth=1.5,
                facecolor=NEON_GREEN, edgecolor='white', zorder=25
            )
            ax.add_patch(rect)
            ax.text(cx, y_top - LEAF_H / 2,
                    f"{ohm:g} Ω\n{p_val:.1f}W",
                    color='black', fontsize=9, ha='center', va='center',
                    fontweight='bold', zorder=26)

        # 7. Disegna cerchi P/S — DOPO speaker, zorder massimo
        def draw_op_circles(n):
            if n[0] == 'leaf': return
            cx, cy = pos[id(n)]
            label = n[0]
            color = '#B200FF' if label == 'P' else '#FF0000'
            ax.add_patch(plt.Circle((cx, cy), NODE_R,
                                    color=color, ec='white', lw=1.5, zorder=40))
            ax.text(cx, cy, label, color='white', fontsize=10,
                    ha='center', va='center', fontweight='bold', zorder=41)
            for c in n[1]: draw_op_circles(c)

        draw_op_circles(flat_node)

        ax.axis('off')
        all_x = [pos[i][0] for i in pos]
        all_y = [pos[i][1] for i in pos]
        margin_x = 3.0; margin_y = 1.5
        ax.set_xlim(min(all_x) - margin_x, max(all_x) + margin_x)
        ax.set_ylim(min(all_y) - LEAF_H - margin_y, amp_y + margin_y)
        ax.set_aspect('equal')
