...
Versione V3 — Keygen-style GUI con:
 ...
 - mantiene audio opzionale, versione completa, annulla, esport PNG
"""
...
def calc_powers(node, Vtot=None, Itot=None):
    """Restituisce dict {valore_resistenza: watt_su_di_lei}"""
    t = node[0]
    if t == 'leaf':
        R = float(node[1])
        if Vtot is not None:
        ...
        a, b = node[1], node[2]
        Ra, Rb = expr_value_and_str(a)[0], expr_value_and_str(b)[0]
        Rt = Ra + Rb
        if Itot is None:
            Itot = Vtot / Rt
        Pa = calc_powers(a, Itot=Itot)
        Pb = calc_powers(b, Itot=Itot)
        d = {}
        d.update(Pa); d.update(Pb)
        return d
    elif t == 'P':
        a, b = node[1], node[2]
        Ra, Rb = expr_value_and_str(a)[0], expr_value_and_str(b)[0]
        if Vtot is None:
            Vtot = Itot / (1/Ra + 1/Rb)
        Pa = calc_powers(a, Vtot=Vtot)
        Pb = calc_powers(b, Vtot=Vtot)
        d = {}
        d.update(Pa); d.update(Pb)
        return d
...
watt_entry = make_entry(left, "Potenza ingresso (W):", "100")
...
def plot_tree_matplotlib(node, title=None, Pin=None):
    if node is None:
        return None

    root.update_idletasks()
    w_px = max(canvas_widget.winfo_width(), 200)
    h_px = max(canvas_widget.winfo_height(), 120)
    dpi = 100
    fig_w = w_px / dpi
    fig_h = h_px / dpi
    ...
    # Calcolo delle posizioni (ricorsivo)
    def assign(n, path="", depth=0):
        t = n[0]
        if t == 'leaf':
            x = counter['x']
            positions[path] = (x, -depth)
            labels[path] = f"{float(n[1]):g}Ω"
            counter['x'] += 1
            return positions[path]
        left = n[1]
        right = n[2]
        lpos = assign(left, path + "L", depth + 1)
        rpos = assign(right, path + "R", depth + 1)
        x = (lpos[0] + rpos[0]) / 2.0
        positions[path] = (x, -depth)
        labels[path] = n[0]
        return positions[path]
    ...
    # Calcolo distribuzione potenza se Pin è definito
    powers_map = {}
    if Pin and Pin > 0:
        Rt = expr_value_and_str(node)[0]
        Vt = (Pin * Rt) ** 0.5

        def calc(node, path="", Vtot=None, Itot=None):
            t = node[0]
            if t == 'leaf':
                R = float(node[1])
                if Vtot is not None:
                ...
                if Itot is None:
                    Itot = Vtot / Rt_local
                calc(a, path + "L", Itot=Itot)
                calc(b, path + "R", Itot=Itot)
            elif t == 'P':
                a, b = node[1], node[2]
                Ra, Rb = expr_value_and_str(a)[0], expr_value_and_str(b)[0]
                if Vtot is None:
                    Vtot = Itot / (1 / Ra + 1 / Rb)
                calc(a, path + "L", Vtot=Vtot)
                calc(b, path + "R", Vtot=Vtot)
    ...
    def draw(n, path=""):
        ...
        if t != 'leaf':
            ...
            for tag, child in zip(("L", "R"), (left, right)):
                cx, cy = positions[path + tag]
                ax.plot([x, cx], [y, cy], color=branch_color, linewidth=1.5)
                draw(child, path + tag)

        ax.scatter([x], [y], s=100, color=NEON_GREEN)
        text_main = labels[path]

        # Aggiunge i watt se disponibili
        if t == 'leaf' and path in powers_map:
            Pw = powers_map[path]
            text_main += f"\n{Pw:.1f} W"
    ...
    ax.axis('off')
    if title:
        ax.set_title(title, color=NEON_GREEN)
    fig.tight_layout()
    return fig
...
def update_canvas_figure(fig_local):
    ...
    if fig_local is None:
        return
    current_fig = fig_local  # <-- salva qui
    root.update_idletasks()
...
def show_results(results):
    ...
    for idx, r in enumerate(results):
        ...
        tree.insert('', 'end', iid=str(idx),
                    values=(",".join(speakers_str), r['expr'], f"{r['value']:.6f}"))

    status_label.config(text=f"Trovate {len(results)} soluzioni.")

    # Disegna il primo solo se vuoi, ma aggiorna current_fig
    if results and draw_schema_var.get():
        ...
        if sel:
            ...
            node = entry['tree']
            try:
                Pin = float(watt_entry.get())
            except Exception:
                Pin = 0
            fig_local = plot_tree_matplotlib(node, title=entry['expr'], Pin=Pin)
...
def export_png():
    """Esporta lo schema attualmente visualizzato nel canvas come immagine PNG"""
    global current_fig
    try:
        # Controlla che esista una figura attiva
        if current_fig is None:
            messagebox.showwarning("Attenzione", "Nessuno schema da esportare.")
            return

        fig = current_fig

        # Ricava la formula del circuito se disponibile
        title = ""
        try:
        ...
        # Pulisce la formula per usarla come nome file
        if title:
            ...
            safe_title = safe_title.replace("{", "_").replace("}", "_")
            safe_title = safe_title.replace(":", "_").replace("*", "_")
            base_name = f"{safe_title}.png"
        else:
            base_name = "schema.png"

        # Finestra di salvataggio con nome suggerito
        filename = filedialog.asksaveasfilename(
            initialfile=base_name,
            defaultextension=".png",
            filetypes=[("Immagine PNG", "*.png")],
            title="Salva schema come PNG"
        )
        if not filename:
            return

        # Salvataggio PNG
        fig.savefig(filename, dpi=300, facecolor='black', bbox_inches='tight')
        messagebox.showinfo("Esportazione completata", f"Schema salvato come:\n{filename}")

    except Exception as e:
        messagebox.showerror("Errore", f"Errore durante l'esportazione PNG:\n{e}")
...
tk.Button(btns_frame, text="Esporta PNG", command=export_png, bg=ENTRY_BG, fg=NEON_GREEN, bd=0, relief="flat").pack(fill="x", pady=2)
